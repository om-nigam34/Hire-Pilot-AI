(() => {
  ...
  async function handleAnalyzeSubmit(e) {
    ...
    try {
      const res = await fetch("/api/analyze", { method: "POST", body: formData, credentials: "same-origin" });
  ...
  async function loadSessionIntoView(sessionId) {
    ...
    try {
      const res = await fetch(`/api/session/${sessionId}`, { credentials: "same-origin" });
      const data = await res.json();
